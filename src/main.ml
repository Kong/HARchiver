...
let command =
  Command.basic
    ~summary:"Universal lightweight analytics layer for Galileo"
    ~readme:(fun () -> "Portable, fast and transparent proxy.\n
      It lets HTTP/HTTPS traffic through and streams datapoints to Galileo\n
    ...
    Command.Spec.(
      ...
      +> flag "debug" no_arg ~doc:" Print generated HARs once they've been flushed to Galileo."
      ...
      +> flag "host" (optional string) ~doc:" (Dev option). Overrides the Galileo host"
      +> flag "port" (optional int) ~doc:" (Dev option). Overrides the Galileo port"
